...
def main():
    ...
    for epoch in range(max_epochs):
        ...
        with torch.set_grad_enabled(False):
            ...
            print('[%d] loss: %.3f, Train accuracy: %.3f' %(epoch + 1, running_loss / (i+1), np.mean(auc)), end=", ")
        running_loss = 0.0
        ...
        with torch.set_grad_enabled(False):
            ...
            print('Test accuracy: %.3f' %(np.mean(auc)))

        if(epoch%5 == 0):
            torch.save({
                'epoch': epoch,
                'model_state_dict': net.state_dict(),
                'model': net}, 'models_porsche/pybasset_{0:03d}.pwf'.format(epoch))
