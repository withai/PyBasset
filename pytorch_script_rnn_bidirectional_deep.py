...
class Basset(nn.Module):
    ...
    def __init__(self):
        ...
        self.layer4 = nn.Sequential(
            nn.Conv1d(in_channels=200, out_channels=100, kernel_size=3),
            #nn.BatchNorm1d(num_features=100),
            nn.ReLU())
            #nn.MaxPool1d(kernel_size=3))

        self.layer5 = nn.LSTM(input_size=8, hidden_size=8, num_layers=2, batch_first=True, dropout=0.2, bidirectional=True)

        self.fc1 = nn.Linear(in_features=1600, out_features=1000)
        self.relu4 = nn.ReLU()
...
def main():
    ...
    for epoch in range(max_epochs):
        ...
        torch.save({
            ...
            'auc_train': auc_train,
            'auc_test': auc_test,
            'running_loss': running_loss}, 'models_lamborghini_rnn_bidirectional_deep/pybasset_{0:03d}.pwf'.format(epoch))
